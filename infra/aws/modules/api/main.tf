...
# Public routes: invitation validate (no JWT — unauthenticated users checking invite links)
resource "aws_apigatewayv2_route" "public_invitation_validate" {
  api_id    = aws_apigatewayv2_api.main.id
  route_key = "GET /api/invitations/validate"
  target    = "integrations/${aws_apigatewayv2_integration.alb.id}"
}

# Invitation accept — server validates JWT manually (user just signed up)
resource "aws_apigatewayv2_route" "public_invitation_accept" {
  api_id    = aws_apigatewayv2_api.main.id
  route_key = "POST /api/invitations/accept"
  target    = "integrations/${aws_apigatewayv2_integration.alb.id}"
}
...
resource "aws_apigatewayv2_route" "public_teams_webhook" {
  api_id    = aws_apigatewayv2_api.main.id
  route_key = "POST /api/channels/teams/webhook"
  target    = "integrations/${aws_apigatewayv2_integration.alb.id}"
}
