...
- `TenantSettings.channel_routing_overrides: dict[str, str]` — per-sender
  routing override map. Keyed `"{channel}:{sender_id}"` (sender_id is
  digits-only for whatsapp/telegram, raw id for teams). Value is the skill
  name to dispatch directly, bypassing tier 1/2/3 routing. Used by the
  platform to wire e.g. a dedicated WhatsApp phone number straight to a
  voice synthesis skill.

### Notes

- Additive field with a default-empty dict — existing serialized settings
